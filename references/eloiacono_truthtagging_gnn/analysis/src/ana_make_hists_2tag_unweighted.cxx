...
int main(int argc, char* argv[]) {
    ...
    // for pdg info
    TTreeReaderValue<std::vector<int>> pdg_parton(data_reader, "pdg_parton");
    ...
    TH1F_custom *dR_hist_allperm_NN = new TH1F_custom("dR_hist_allperm_NN", "deltaR (All Permutations)", 23, 0.4, 4.0);
    ...
    TH1F_custom *dR_hist_allperm_custom = new TH1F_custom("dR_hist_allperm_custom", "deltaR (All Permutations)", 23, 0.4, 4.0);



    double eventMass, eventdR;

    int count_discarded_events_pdgid = 0;
...
//        // check if we have pileup jet (with pdg id)
//        if (skip == false){
//            for (int i=0; i<N; i++){
//                if (pdg_parton->at(i) == -1){
//                    skip = true;
//                    count_discarded_events_pdgid++;
//                    continue;
//                }
//            }
//        }
        ...
        if (num_tagged ==  2){
            ...
            mass_hist_direct->Fill(*mJ, 1.0, jet_truthflav->at(tag_num[0]), jet_truthflav->at(tag_num[1]));
            dR_hist_direct->Fill(deltaR, 1.0, jet_truthflav->at(tag_num[0]), jet_truthflav->at(tag_num[1]));
        }
        ...
        for (int i=0; i<N; i++){
            for (int j=i+1; j<N; j++){
                ...
                mass_hist_allperm_NN->Fill(*mJ, per_eff_NN * (1.0), jet_truthflav->at(i), jet_truthflav->at(j));
                dR_hist_allperm_NN->Fill(dR, per_eff_NN * (1.0), jet_truthflav->at(i), jet_truthflav->at(j));

                mass_hist_allperm_custom->Fill(*mJ, per_eff_custom * (1.0), jet_truthflav->at(i), jet_truthflav->at(j));
                dR_hist_allperm_custom->Fill(dR, per_eff_custom * (1.0), jet_truthflav->at(i), jet_truthflav->at(j));
            }
        }
        ...
        mass_hist_NN->Fill(eventMass, weight_NN * (1.0), flavs[0], flavs[1]);
        dR_hist_NN->Fill(eventdR, weight_NN * (1.0), flavs[0], flavs[1]);
        ...
        mass_hist_custom->Fill(eventMass, weight_custom * (1.0), flavs[0], flavs[1]);
        dR_hist_custom->Fill(eventdR, weight_custom * (1.0), flavs[0], flavs[1]);
    ...
    // discarded event counts
    std::cout << "Total discarded events based on pdgid cut : " << count_discarded_events_pdgid << std::endl;
